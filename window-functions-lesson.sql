...
SELECT
    ...
    , ps.AdmittedDate
    , ps.Tariff
    , COUNT(*) OVER () AS TotalCount -- window over () with nothing specified in brackets so counts all the rows in the table --aggregate function doesn't need group by
...
SELECT
  ...
  , COUNT(*) OVER (PARTITION BY ps.Hospital) AS HospitalCount -- create a window over those rows with the same hospital as the current row --creating an aggregate count of the hospital
  , COUNT(*) OVER (PARTITION BY ps.Ward) AS WardCount --counting total for wards
  , COUNT(*) OVER (PARTITION BY ps.Hospital , ps.Ward) AS HospitalWardCount --using both totals of hospital and ward
FROM
    PatientStay ps
ORDER BY
    ps.PatientId;
...
SELECT
    ...
    , ps.Tariff
    , ps.Ward
    , SUM(ps.Tariff) OVER () AS TotalTariff --summing the total of Tariff column
    , SUM(ps.Tariff) OVER (PARTITION BY ps.Ward) AS WardTariff --summing up total tariff by ward
    , 100.0 * ps.Tariff / SUM(ps.Tariff) OVER () AS PctOfAllTariff --percentage of grand total calculation
    , 100.0 * ps.Tariff / SUM(ps.Tariff) OVER (PARTITION BY ps.Ward) AS PctOfWardTariff --percentage of group calculation
FROM
    PatientStay ps
ORDER BY
    ps.Ward
...
SELECT
  ...
  , NTILE(4) OVER (ORDER BY ps.Tariff DESC) AS PatientIdDecile -- groups the data in 4 groups ordered by tariff
FROM
    PatientStay ps
ORDER BY
    ps.Tariff DESC;
...
SELECT
    ...
    , ps.Tariff
     ...
     , SUM(ps.Tariff) OVER (ORDER BY ps.AdmittedDate) AS RunningTariff -- produces a cumulative total
     , ROW_NUMBER() OVER (PARTITION BY DATENAME(MONTH, ps.AdmittedDate) ORDER BY ps.AdmittedDate) AS MonthIndex
     , SUM(ps.Tariff) OVER (PARTITION BY DATENAME(MONTH, ps.AdmittedDate) ORDER BY ps.AdmittedDate) AS MonthToDateTariff -- calculates month to date value
FROM
    PatientStay ps
WHERE
    ps.Hospital = 'Oxleas'
